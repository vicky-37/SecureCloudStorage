...
const Dashboard = () => {
  ...
  const fetchFiles = async () => {
    try {
      setLoading(true);
      const response = await fetch("13.53.37.186:5000/files", {
  ...
  const handleUpload = async () => {
    ...
    try {
      setLoading(true);
      const response = await fetch("13.53.37.186:5000/upload", {
        method: "POST",
        headers: { Authorization: `Bearer ${getToken()}` },
        body: formData
      });
  ...
  const handleDownload = async (filename) => {
    ...
    try {
      const response = await fetch(`13.53.37.186:5000/download/${filename}`, {
        method: "GET",
